...
class VoiceAssistantService {
  ...
  Future<VoiceProcessResult> processVoice(String audioPath) async {
    ...
    if (token == null) {
      return VoiceProcessResult.error('Not authenticated. Please wait for backend sync.');
    }
    ...
    try {
      ...
      debugPrint('VoiceAssistantService: Sending request ($fileSize bytes, $mediaSubtype)...');
      ...
      debugPrint('VoiceAssistantService: Response status: ${streamedResponse.statusCode}');
      ...
      if (streamedResponse.statusCode != 200) {
        ...
        try {
          ...
          return VoiceProcessResult.error(
            errorData['message'] ?? errorData['detail'] ?? 'Server error (${streamedResponse.statusCode})',
          );
        } catch (_) {
          return VoiceProcessResult.error('Server error (${streamedResponse.statusCode})');
        }
      }
  ...
  VoiceProcessResult _parseAudioResponse(http.Response response) {
    ...
    if (metadataHeader != null && metadataHeader.isNotEmpty) {
      try {
        ...
        debugPrint('VoiceAssistantService: Failed to parse X-Voice-Metadata: $e');
      }
    }
    ...
    debugPrint('VoiceAssistantService: ✅ Audio response — intent=$intent, action=$action, ${response.bodyBytes.length} bytes');
  ...
  VoiceProcessResult _parseJsonResponse(http.Response response) {
    try {
      ...
      debugPrint('VoiceAssistantService: ✅ JSON fallback — intent=${d['intent']}, action=${d['action']}');
      ...
      return VoiceProcessResult(
        ...
        action: d['action'],
        data: d['data'] != null ? Map<String, dynamic>.from(d['data'] as Map) : null,
      );
    } catch (e) {
  ...
  Future<VoiceProcessResult> _retryProcessVoice(String audioPath) async {
    ...
    try {
      ...
      return VoiceProcessResult.error('Retry failed: ${e.toString().split('\n').first}');
    }
  }
